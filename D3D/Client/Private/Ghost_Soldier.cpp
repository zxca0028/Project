...
CGhost_Soldier::CGhost_Soldier(LPDIRECT3DDEVICE9 pGraphic_Device)
...
HRESULT CGhost_Soldier::NativeConstruct_Prototype()
{
...
HRESULT CGhost_Soldier::NativeConstruct(void* pArg)
{
	...
	IconInfo.WorldMatrix = m_pTransform->Get_WorldMatrix();
...
_int CGhost_Soldier::Tick(_double DeltaTime)
{
	__super::Tick(DeltaTime);

	m_vPos = _float4(m_vMyPos, 1.f);

	m_pCollider->Collision_OBB(m_pPlayer_N);
...
_int CGhost_Soldier::Late_Tick(_double DeltaTime)
{
...
HRESULT CGhost_Soldier::Set_ConstTable()
{
...
HRESULT CGhost_Soldier::Add_Components(void* pArg)
{
	...
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Mesh_GhostSoldier"), TEXT("Com_Mesh"), (CComponent**)&m_pMesh))) { return E_FAIL; }
...
void CGhost_Soldier::Monster_Behavior(_double DeltaTime)
{
	...
	case CHASE:
		...
		if (m_fDist > 1.0f)
		{
			...
			m_pTransform->Chase_Target(m_vPlayerPos, DeltaTime * 1.4f);
	...
	case KNOCKBACK:
	{
		...
		if (m_fTimeAcc < 1.f)
		{
			m_fDelta = (powf(512, -m_fTimeAcc)) / 10.f;

			if (true == m_bisIn)
			{
				m_pTransform->Move_Back(m_fDelta);
			}
		}
	...
	case DIE2:
		...
		{
			CreateEffect(TEXT("Prototype_Effect_FinalFire"), 4.f);
		...
		if (m_fTimeAcc > 3.f)
		{
			...
			pGameInstance->Add_DeleteContainer(LEVEL_GAMEPLAY, TEXT("Layer_Ghost_Soldier"), this);
...
_bool CGhost_Soldier::Hit_Q(_double DeltaTime)
{
...
_bool CGhost_Soldier::Hit_W(_double DeltaTime)
{
	...
	else if (m_fTimeAcc_W > 0.7f && m_fTimeAcc_W < 0.8f && true == CSkill_Manager::Get_Instance()->IsActive_W())
	{
		if (m_pCollider->Collision_OBB(m_pPlayer_W))
		{
			if (false == m_bLock_W5)
			{
				...
				CreateEffect(TEXT("Prototype_Effect_Lightning"), 2.f, false, true);
...
_bool CGhost_Soldier::Hit_E(_double DeltaTime)
{
...
_bool CGhost_Soldier::Hit_R(_double DeltaTime)
{
...
_bool CGhost_Soldier::Hit_N(_double DeltaTime, _bool bActionChange)
{
...
_bool CGhost_Soldier::Hit_F(_double DeltaTime)
{
...
void CGhost_Soldier::Time_Initailize(_double DeltaTime)
{
...
void CGhost_Soldier::CreateEffect(const _tchar* pPrototypeTag, _float fScale, _bool isRandom, _bool isPtr)
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
...
void CGhost_Soldier::Attack_Player(_float fTime, _float fDamage)
{
...
CGhost_Soldier* CGhost_Soldier::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CGhost_Soldier* pInstance = new CGhost_Soldier(pGraphic_Device);
...
CGameObject* CGhost_Soldier::Clone(void* pArg)
{

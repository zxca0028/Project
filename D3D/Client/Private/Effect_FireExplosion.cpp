...
CEffect_FireExplosion::CEffect_FireExplosion(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CBlender(pGraphic_Device)
{}

CEffect_FireExplosion::CEffect_FireExplosion(const CEffect_FireExplosion& rhs)
	: CBlender(rhs)
{}

HRESULT CEffect_FireExplosion::NativeConstruct_Prototype()
{
...
HRESULT CEffect_FireExplosion::NativeConstruct(void* pArg)
{
...
_int CEffect_FireExplosion::Tick(_double TimeDelta)
{
	__super::Tick(TimeDelta);

	m_fFrame += 29.f * TimeDelta;

	if (m_fFrame >= 30.f && false == m_bLock)
	{
		m_bLock = true;

		CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);

		_float4 vPos = _float4(m_pTransform->Get_State(CTransform::STATE_POSITION), 0.f);
...
_int CEffect_FireExplosion::Late_Tick(_double TimeDelta)
{
...
HRESULT CEffect_FireExplosion::Render()
{
...
HRESULT CEffect_FireExplosion::Set_ConstTable()
{
...
HRESULT CEffect_FireExplosion::Add_Components()
{
	...
	if (FAILED(CGameObject::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Texture_FireExplosion"), TEXT("Com_Texture"), (CComponent**)&m_pTexture))) { return E_FAIL; }

	return S_OK;
}

CEffect_FireExplosion* CEffect_FireExplosion::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CEffect_FireExplosion* pInstance = new CEffect_FireExplosion(pGraphic_Device);
...
CGameObject* CEffect_FireExplosion::Clone(void* pArg)
{
	CEffect_FireExplosion* pInstance = new CEffect_FireExplosion(*this);
...
void CEffect_FireExplosion::Free()
{
	__super::Free();

...
CEffect_Arcane::CEffect_Arcane(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CBlender(pGraphic_Device)
{}

CEffect_Arcane::CEffect_Arcane(const CEffect_Arcane& rhs)
	: CBlender(rhs)
{}

HRESULT CEffect_Arcane::NativeConstruct_Prototype()
{
...
HRESULT CEffect_Arcane::NativeConstruct(void* pArg)
{
...
_int CEffect_Arcane::Tick(_double TimeDelta)
{
	__super::Tick(TimeDelta);

	m_fFrame += 41.f * TimeDelta * 2.f;
	...
	if (m_fFrame >= 41.f)
	{
		...
		pGameInstance->Add_DeleteContainer(LEVEL_GAMEPLAY, TEXT("Layer_Arcane"), this);
...
_int CEffect_Arcane::Late_Tick(_double TimeDelta)
{
...
HRESULT CEffect_Arcane::Set_ConstTable()
{
...
HRESULT CEffect_Arcane::Add_Components()
{
	...
	if (FAILED(CGameObject::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Texture_Arcane"), TEXT("Com_Texture"), (CComponent**)&m_pTexture))) { return E_FAIL; }

	return S_OK;
}

CEffect_Arcane* CEffect_Arcane::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CEffect_Arcane* pInstance = new CEffect_Arcane(pGraphic_Device);
...
CGameObject* CEffect_Arcane::Clone(void* pArg)
{
	CEffect_Arcane* pInstance = new CEffect_Arcane(*this);
